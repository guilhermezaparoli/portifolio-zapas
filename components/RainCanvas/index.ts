...
const RainCanvas = () => {
  ...
  useEffect(() => {
    ...
    const mainLoop = () => {
      ...
      const currentBackgroundColor = blendColors(nightColor, dayColor, transitionProgress);
    ...
    const lamp = () => {
      ...
      const grdOuterHigh = ctx.createLinearGradient(lampX, lampY, lampX, lampY + 290);
      ...
      const grdOuterLow = ctx.createLinearGradient(lampX, lampY, lampX, lampY + 290);
    ...
    const drawSun = (progress: number) => {
      ...
      // Outer glow
      const outerGlow = ctx.createRadialGradient(sunX, sunY, 0, sunX, sunY, sunRadius + 60);
      ...
      // Inner Glow
      const innerGlow = ctx.createRadialGradient(sunX, sunY, 0, sunX, sunY, sunRadius * 0.5);
